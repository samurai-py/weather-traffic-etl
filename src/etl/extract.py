...
from operators.extractors.directions import get_routes

def run_extract(weather_output_path='../../data/weather_raw_data.csv', routes_output_path='../../data/routes_raw_data.csv'):
    ...
    routes_result = get_routes(dataframe=cities_df, num_cities=3)
    
    weather_result.to_csv(weather_output_path, index=False)   
    
    # Salvar o DataFrame das rotas em um arquivo CSV
    routes_result.to_csv(routes_output_path, index=False)     
    
    return weather_output_path, routes_output_path
