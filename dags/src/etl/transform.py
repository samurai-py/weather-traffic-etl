...
def write_records(path='/usr/local/airflow/dags/src/data/records.csv'):
    
    r_uuid = generate_uuid()
    records_data = pd.DataFrame({'uuid': [r_uuid]})
    records_data.to_csv(path, mode='a', header=None, index=True)
    
def read_records(path='/usr/local/airflow/dags/src/data/records.csv'):
    write_records()
...
def run_transform(weather_output_path='/usr/local/airflow/dags/src/data/weather_cleaned_data.csv', directions_output_path='/usr/local/airflow/dags/src/data/directions_cleaned_data.csv'):
